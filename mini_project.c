...
int engine_controller(void){
    ...
    {printf(" you can not set up engine temperature controller, its locked \n");
    state();
    }
    #else
    if (continue_running==1 ){
	if( (! Engine_controller)&&(vehicle_speed==30) && (WITH_ENGINE_TEMP_CONTROLLER)){
	...
	else if(WITH_ENGINE_TEMP_CONTROLLER){
